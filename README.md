...
anfitrión. La sospecha es saturación térmica del disipador —la sesión lenta arrancó
inmediatamente después del banco de `dgemm`, la rápida tras un rato de reposo— pero
**no se comprobó, y queda como limitación del trabajo**.
...
- **Queda sin explicar un 56 % de variación entre sesiones** con configuración idéntica y
  el equipo enchufado. Es la mayor fuente de incertidumbre del trabajo. La hipótesis es
  saturación térmica del disipador, pero no se comprobó; haría falta instrumentar la
  temperatura del paquete, que WSL2 tampoco expone.
- **WSL2 impide fijar procesos a cores** (*pinning*), porque reporta una topología
  sintética. En un procesador híbrido esa técnica probablemente daría bastante, y es la
  principal palanca que quedó sin probar.
...
- N = 30.000 está limitado por los 12 GB asignados a WSL2, no por el algoritmo. Con más
  memoria el segundo factor de la eficiencia seguiría subiendo.
...
│   └── frecuencia/            # muestreo de frecuencia y corridas definitivas
├── scripts/
